...
void TaskPlannerNode::attemptGenerate()
{
    ...
    if (!gates_received_ || !start_received_ || !graph_received_ || !victims_received_ || !obstacles_received_ || !borders_received_ || !victimsTimeoutReceived_
            || data_generated_)
    {
...
void processSlowSegments(AStarGreedy& planner,
                         ...
                         double timeLimit)
{
    //double basePenalty = 60.0; // You can tune this
    //double perSegmentPenalty = basePenalty / std::max(1.0, (double)slowSegments.size());
    double perSegmentPenalty = 1;

    std::vector<Point> visitedVictims;
    ...
    {
        Point vp(v.getX(), v.getY());
        planner.addVictimPenalty(vp, perSegmentPenalty);
        std::cout << "Added victim penalty for point (" << vp.getX() << ", " << vp.getY() << ") with penalty " << perSegmentPenalty << std::endl;
    }
}
...
std::vector<Point> TaskPlannerNode::generatePath(int attempt)
{
    if (start_received_ && gates_received_ && graph_received_ && victims_received_ && obstacles_received_ && borders_received_ && victimsTimeoutReceived_ ) // Ensure all data is available
    {
        ...
        AStarGreedy planner(graph, victims_, start, goal, pathLimit, obstacles_, borders_);
